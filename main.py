...
from models import AIGameUpdate, CharSelection, PlayerGameUpdate
...
async def start_game(char_selection: CharSelection):
    ...
    ai_game_update = ai_game_update_parser.parse(response)
...
async def update_game(player_game_update: PlayerGameUpdate):
    ...
    try:
        ...
        fix_parser = OutputFixingParser.from_llm(parser=ai_game_update_parser, llm=conversation.llm)
        ai_game_update = fix_parser.parse(response)

    # send AI's response to the client, along with the previous messages for chaining
    ai_game_update.previous_messages = messages_to_dict(
        conversation.memory.chat_memory.messages
    )
